...
public class FirmaNewFragment extends AbstractInputFragment {

	public static FirmaNewFragment newInstance() {
		FirmaNewFragment fragment = new FirmaNewFragment();
		Bundle args = new Bundle();
		args.putBoolean(ARG_KEY_IS_EDIT, false);
		args.putInt(ARG_KEY_LAYOUT_ID, R.layout.gen_fragment_firma_new);
	...
	public static FirmaNewFragment newInstance(String initColumnName, long initValue) {
		FirmaNewFragment fragment = newInstance();
		Bundle args = fragment.getArguments();
		args.putStringArray(ARG_KEY_NEW_INIT_COLUMN_NAMES, new String[] {initColumnName});
		args.putLongArray(ARG_KEY_NEW_INIT_LONG_VALUES, new long[] {initValue});
		fragment.setArguments(args);
		return fragment;
	}
	...
	protected void initializeSpinners() {
		...
		//spNadfirma
		{
			...
			Cursor cursorEntity = getActivity().getContentResolver().query(
					FirmaContentProvider.CONTENT_URI, projection, null,
					null, null);
	...
	protected void putDataInViewControls(Cursor cursor) {
		...
		//naseljenoMesto
		{
			int columnIndex = cursor.getColumnIndex(FirmaTable.COLUMN_ID_NASELJENO_MESTO);
			if(columnIndex != -1){
				Spinner spinner = (Spinner) getView().findViewById(R.id.spNaseljenoMesto);
				ViewUtils.selectSpinnerItemById(spinner, cursor.getLong(columnIndex));
			}
		}
		//nadfirma
		{
			int columnIndex = cursor.getColumnIndex(FirmaTable.COLUMN_ID_NADFIRMA);
			if(columnIndex != -1){
				Spinner spinner = (Spinner) getView().findViewById(R.id.spNadfirma);
				ViewUtils.selectSpinnerItemById(spinner, cursor.getLong(columnIndex));
			}
